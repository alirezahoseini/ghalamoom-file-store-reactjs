...
export default function SubmitButton({ value, loading }) {
  return (
    <button type='submit' className='text-white p-4 rounded-xl shadow-xl bg-golbehi text-sm hover:bg-gray-4 transition-all duration-300 mt-6 flex justify-center items-center' disabled={loading} >
      {!loading ? (
        <span>{value}</span>
      ) : (
