...
export default function SubmitButton({ value, loading }) {
  return (
    <button type='submit' className='text-white p-4 rounded-xl shadow-xl bg-custom-golbehi-100 text-sm hover:bg-gray-400 transition-all duration-300 mt-5 flex justify-center items-center' disabled={loading} >
      {!loading ? (
        <span>{value}</span>
      ) : (
