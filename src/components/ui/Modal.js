...
export default function Modal({ isShow, onClose, size = 'w-8/12 lg:w-1/4', children }) {
    return (
        <div id='modal' className={`flex w-full max-w-screen h-screen fixed top-0 right-0 bg-gray-900 justify-center items-center bg-opacity-50 z-50 transition-all duration-500 ${isShow ? 'opacity-100 visible pointer-events-auto ' : 'opacity-0 invisible pointer-events-none'}`}>
