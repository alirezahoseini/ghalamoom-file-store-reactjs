        ...
        <div id='mobile-header' className='lg:hidden'>
            ...
            <div id='visable-header' className={`flex justify-between w-full items-center py-5 px-3 transition-colors duration-500 rounded-b-xl ${haderBg} `}>
                ...
                <div className='flex items-center ml-1'>
                    {/* Login button  */}
                    <Link to={dynamicLinks.panel} className=' text-xs bg-blue-600 p-3 flex items-center justify-center rounded-xl mx-2  text-white hover:bg-slate-600'>
                    {`${getCooki('token') !== null ? 'پنل کاربری' : 'ورود | ثبت نام'}`}
                    </Link>
                    {/* Login button  */}
                    {/* Shopping cart */}
                    <ShoppingCart />
