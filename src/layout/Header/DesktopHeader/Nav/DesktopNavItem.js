        ...
        !subMenu ? (
          ...
          <li className={`desktop-nav-item font-yekan-bakh font-bold text-lg flex items-center text-gray-700 relative`}>
            <NavLink to={url} className={` relative px-3 pt-1 rounded-lg  ${isActive =>
              ...
              <i id='bg' className='absolute top-0 right-0 h-full w-full bg-gray-400 opacity-20 rounded-lg transition-all duration-300'></i>
              <span>
                {name}
              </span>
            </NavLink>
            <span className='w-1 h-3 inline-block border-l-2 border-dotted border-gray-300 mr-1'></span>
          </li >
        ) : (
          // With subMenu -----------------------------------------------
            ...
            <div className={`relative px-3 pt-1 rounded-lg w-fit h-full flex items-center ${(isActive =>
              ...
              <div className='sublinks absolute top-0 right-0 pt-14 cursor-default '>
                <div className={` bg-white w-64 rounded-2xl flex flex-col overflow-hidden`}>
                  {subLinks.map((link) => (
                    <NavLink to={link.url} key={'desktop' + link.id} className={`submenu-item hover:bg-gray-200 cursor-pointer font-yekan-bakh p-4 ${isActive =>
                      isActive ? "active" : ""
                      }}`}>
                      {link.name}
                    </NavLink>
